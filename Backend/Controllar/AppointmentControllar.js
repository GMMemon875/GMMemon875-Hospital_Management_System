...
const isConflict = await User.find({
    firstName: doctor_firtName,   // same doctor
...
console.log( isConflict,doctor_firtName,doctor_lastName,department)
if(isConflict.length === 0){
    return next(new ErrorHandler("Doctor Not Found!",400))
}
 if (isConflict.length > 1){
    return next(new ErrorHandler("Doctore Conflict! so Please Contact To hostpital Management and Get Appointment",400))
 }
